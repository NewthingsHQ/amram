  ...
  if (client) {
    ...
    while (client.connected() && currentTime - previousTime <= timeoutTime) {
      ...
      if (client.available()) {
        ...
        if (header.indexOf("GET /firebase") >= 0) {
          HTTPClient http;
          ...
          if (httpCode > 0) {
            ...
            int index = 0;
            while (index != -1) {
              int nextIndex = payload.indexOf(",", index);
              if (nextIndex != -1) {
                String part = payload.substring(index, nextIndex);
                meds.push(part);
                index = nextIndex + 1;
              } else {
                String part = payload.substring(index);
                meds.push(part);
                index = -1;
              }
              }
              for (int i = 0; i < 32; i++) {
                if (meds[i][3] = true) {
                  breakfast.push(meds[i][0]);
                } else if (meds[i][4] = true) {
                  lunch.push(meds[i][0]);
                } else if (meds[i][5] = true) {
                  dinner.push(meds[i][0]);
                } else if (meds[i][6] = true) {
                  night.push(meds[i][0]);
                }
              }
          } else {
